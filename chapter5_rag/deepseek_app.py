...
GRAPH_DIR = BASE_DIR / "deepseek_outputs" / "graph"
...
def terms(text: str) -> list[str]:
    vocab = [
        "分段装配", "装配方式", "正装", "倒装", "侧装", "卧装", "放射式", "插入式",
        "框架式", "散装式", "子分段", "双层底分段", "舷侧分段", "胎架", "平台",
        "肋板", "纵骨", "纵桁", "外板", "甲板", "焊接变形", "反变形", "刚性固定",
        "分段工作图", "完工测量", "分段定位", "吊装", "合拢", "余量", "补偿量",
        "基准线", "中心线",
    ]
    ...
    for item in candidates:
        if item not in stop and item not in result:
            result.append(item)
    return result[:20]
...
class DeepSeekChatClient:
    ...
    def chat(self, messages: list[dict[str, str]], max_tokens: int = 700, temperature: float = 0.0, json_mode: bool = False) -> str:
        body: dict[str, Any] = {
            "model": self.model,
            "messages": messages,
...
class Chapter5DeepSeekRAG:
    ...
    def retrieve_graph(self, question: str, hops: int, limit: int = 24) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        for ent in self.entities:
            score = score_text(q_terms, f"{ent.get('name','')} {ent.get('definition','')}")
            if score > 0:
        ...
        if len(facts) < max(6, limit // 3):
            for rel in self.relations:
                key = (rel["head"], rel["relation"], rel["tail"])
                if key in seen:
                    continue
                score = score_text(q_terms, f"{rel['head']} {rel.get('relation_zh','')} {rel['relation']} {rel['tail']} {rel.get('evidence','')}")
                if score <= 0:
                    continue
                seen.add(key)
                facts.append({**rel, "score": round(score + float(rel.get("confidence") or 0), 4), "type": "graph"})
        facts.sort(key=lambda row: row["score"], reverse=True)
        if not selected and facts:
            names = {item["head"] for item in facts[:limit]} | {item["tail"] for item in facts[:limit]}
            selected = [{**ent, "score": 1.0} for ent in self.entities if ent.get("name") in names][:8]
        return selected, facts[:limit]

    def planner_agent(self, question: str, trace: list[dict[str, Any]]) -> dict[str, Any]:
        started = time.perf_counter()
        fallback = {"intent": "chapter5_deepseek_domain_qa", "keywords": terms(question), "needs_graph": True, "needs_vector": True}
        try:
            raw = self.deepseek.chat(
                [
                    {"role": "system", "content": "你是第五章船体分段装配 RAG 的 PlannerAgent。只输出 JSON。"},
                    {"role": "user", "content": f"分析问题并输出 intent、keywords、needs_graph、needs_vector。问题：{question}"},
                ],
                max_tokens=180,
                json_mode=True,
            )
            payload = json.loads(re.search(r"\{.*\}", raw, flags=re.S).group(0))
            plan = {
            ...
            self.trace_step(trace, "PlannerAgent", "DeepSeek query planning", {"intent": plan["intent"], "keywords": plan["keywords"]}, started)
            return plan
    ...
    def vector_agent(self, question: str, top_k: int, trace: list[dict[str, Any]], needs_vector: bool = True) -> list[dict[str, Any]]:
        ...
        self.trace_step(trace, "VectorAgent", "Qdrant vector + keyword retrieval", {"documents": len(docs), "top_chunks": [doc.get("id") for doc in docs[:5]]}, started)
        return docs
    ...
    def graph_agent(self, question: str, graph_hops: int, trace: list[dict[str, Any]], needs_graph: bool = True) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        self.trace_step(trace, "GraphAgent", "DeepSeek KG entity linking + graph retrieval", {"linked_entities": len(entities), "graph_facts": len(facts)}, started)
    ...
    def fusion_agent(self, docs: list[dict[str, Any]], facts: list[dict[str, Any]], trace: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for doc in docs:
            if (doc.get("chunk_id") or doc.get("id")) in graph_chunks:
        ...
        self.trace_step(trace, "FusionAgent", "evidence merge and graph-boost rerank", {"documents": len(docs), "graph_facts": len(facts)}, started)
        return {"documents": docs, "graph": facts}

    def verifier_agent(self, answer: str, evidence: dict[str, Any], trace: list[dict[str, Any]]) -> dict[str, Any]:
        started = time.perf_counter()
        result = {
            "grounded": bool(evidence.get("documents") or evidence.get("graph")),
            "has_required_sections": all(marker in answer for marker in ("结论", "依据", "引用")),
            "document_evidence": len(evidence.get("documents", [])),
            "graph_evidence": len(evidence.get("graph", [])),
        }
    ...
    def answer(self, question: str, top_k: int = 5, graph_hops: int = 1) -> dict[str, Any]:
        ...
        docs = self.vector_agent(question, top_k, trace, plan.get("needs_vector", True))
        entities, facts = self.graph_agent(question, graph_hops, trace, plan.get("needs_graph", True))
        fused = self.fusion_agent(docs, facts, trace)
        answer_started = time.perf_counter()
        answer = self.generate_answer(question, fused["documents"], fused["graph"])
        self.trace_step(trace, "AnswerAgent", "DeepSeek evidence-grounded answer generation", {"answer_chars": len(answer), "llm": self.deepseek.model}, answer_started)
        ...
        return {
            ...
            "metadata": {
                ...
                "chapter": "第五章 船体分段的装配",
                "graph_provider": "deepseek",
                "graph_entities": self.summary.get("entities"),
                "graph_relations": self.summary.get("relations"),
                "retrieval_mode": "chapter5_deepseek_multi_agent_graph_vector_rag",
                "planner": plan,
    ...
    def generate_answer(self, question: str, docs: list[dict[str, Any]], facts: list[dict[str, Any]]) -> str:
        graph_context = "\n".join(f"- {rel['head']} --{rel.get('relation_zh') or rel['relation']}--> {rel['tail']}；证据：{rel.get('evidence','')}" for rel in facts[:12])
        doc_context = "\n".join(f"- chunk={doc['id']}；位置 {doc.get('page_start')}；{doc.get('text','')[:320]}" for doc in docs[:6])
        if not graph_context and not doc_context:
            return "结论：根据第五章 DeepSeek 知识库暂时无法确定。\n依据：未检索到相关图谱事实或教材片段。\n引用：无。"
        try:
            return clean_model_output(
                self.deepseek.chat(
                    [
                        {"role": "system", "content": "你是船体分段装配工艺课程的第五章知识问答助手。只基于给定证据回答。"},
                        {
                            "role": "user",
...
rag = Chapter5DeepSeekRAG()
app = FastAPI(title="Chapter 5 DeepSeek ShipRAG")
