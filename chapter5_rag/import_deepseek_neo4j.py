...
DEFAULT_GRAPH = CHAPTER5_DIR / "deepseek_outputs" / "graph"
SCOPE = "chapter5_deepseek"
ID_PREFIX = f"{SCOPE}::"
...
def ensure_schema(session) -> None:
    session.run("CREATE CONSTRAINT chapter5_deepseek_entity_id_unique IF NOT EXISTS FOR (e:Chapter5DeepSeekEntity) REQUIRE e.id IS UNIQUE")
    session.run("CREATE INDEX chapter5_deepseek_entity_name IF NOT EXISTS FOR (e:Chapter5DeepSeekEntity) ON (e.name)")
    session.run("CREATE INDEX chapter5_deepseek_entity_type IF NOT EXISTS FOR (e:Chapter5DeepSeekEntity) ON (e.type)")
    session.run("CREATE INDEX chapter5_deepseek_entity_scope IF NOT EXISTS FOR (e:Chapter5DeepSeekEntity) ON (e.scope)")


def clear_deepseek(session) -> None:
    session.run("MATCH (n:Chapter5DeepSeekEntity) DETACH DELETE n")
...
def import_entities(session, entities: list[dict[str, Any]], batch_size: int = 300) -> None:
    for start in range(0, len(entities), batch_size):
        ...
        session.run(
            """
            UNWIND $batch AS row
            MERGE (e:Chapter5DeepSeekEntity {id: row.id})
    ...
    for typ, label in TYPE_LABELS.items():
        session.run(
            f"""
            MATCH (e:Chapter5DeepSeekEntity {{type: $typ}})
            SET e:{label}
            """,
            typ=typ,
        )
    ...
    for rel_type, rows in grouped.items():
        for start in range(0, len(rows), batch_size):
            ...
            result = session.run(
                f"""
                UNWIND $batch AS row
                MATCH (h:Chapter5DeepSeekEntity {{id: row.head_id}})
                MATCH (t:Chapter5DeepSeekEntity {{id: row.tail_id}})
...
def collect_stats(session) -> dict[str, int]:
    stats = session.run(
        """
        MATCH (n:Chapter5DeepSeekEntity)
        WITH count(n) AS nodes
        MATCH (:Chapter5DeepSeekEntity)-[r {scope: $scope}]->(:Chapter5DeepSeekEntity)
        RETURN nodes, count(r) AS relationships
        """,
        scope=SCOPE,
    ).single()
    return {"chapter5_deepseek_nodes": stats["nodes"], "chapter5_deepseek_relationships": stats["relationships"]}


def main() -> None:
    parser = argparse.ArgumentParser(description="Import chapter 5 DeepSeek KG into Neo4j without touching other graphs.")
    parser.add_argument("--graph-dir", type=Path, default=DEFAULT_GRAPH)
    parser.add_argument("--clear-deepseek", action="store_true", help="Delete only Chapter 5 DeepSeek nodes before import.")
    parser.add_argument("--stats-only", action="store_true", help="Only print Chapter 5 DeepSeek graph stats in Neo4j.")
    ...
    with driver:
        with driver.session(database=database) as session:
            ...
            if args.clear_deepseek:
                print("Clearing only Chapter 5 DeepSeek Neo4j subgraph...")
                clear_deepseek(session)
            if args.stats_only:
            ...
            print(f"Importing Chapter 5 DeepSeek entities: {len(entities)}")
            import_entities(session, entities)
            print(f"Importing Chapter 5 DeepSeek relations: {len(relations)}")
